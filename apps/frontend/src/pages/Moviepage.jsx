        ...
        <div className="min-h-screen bg-[#181818] text-white">
            ...
            >
                ...
                <div className="absolute bottom-6 left-64 z-10">
                    ...
                    <SaveButton/>
                    <LikeButton/>
                    <Link to={`https://www.youtube.com/watch?v=${youtubelink}`} target="_blank">
                    <button className="flex justify-center items-center bg-red-600 hover:bg-red-900 text-white py-3 px-4 rounded-full cursor-pointer text-sm md:text-base ml-3"><Play className="mr-2 w-4 h-5 md:w-5 md:h-5"/> Watch Trailer</button>
                    </Link>
                    </div>
                </div>
            </div>
            ...
            <div className="p-8">
                ...
                <div className="bg-[#232323] rounded-lg shadow-lg p-6 flex flex-col md:flex-row gap-8">
                    <div className="flex-1">
                        <ul className="text-gray-300 space-y-3">
                            ...
                            <li>
                                ...
                                <span className="ml-2 text-gray-400">{movie.release_date}</span>
                            </li>
                            <li>
                                <span className="font-semibold text-white">Original Language: </span>
                                <span className="ml-2 text-gray-400">{movie.original_language?.toUpperCase()}</span>
                            </li>
                            <li>
                                <span className="font-semibold text-white">Budget: </span>
                                <span className="ml-2 text-gray-400">{movie.budget? `$${movie.budget.toLocaleString()}` : 'N/A'}</span>
                            </li>
                            <li>
                                <span className="font-semibold text-white">Revenue: </span>
                                <span className="ml-2 text-gray-400">{movie.budget? `$${movie.revenue.toLocaleString()}` : 'N/A'}</span>
                            </li>
                              <li>
                                <span className="font-semibold text-white">Production Companies: </span>
                                <span className="ml-2 text-gray-400">{movie.production_companies&& movie.production_companies.length>0? movie.production_companies.map((c)=>c.name).join(", "): "N/A"}</span>
                            </li>
                            <li>
                                <span className="font-semibold text-white">Spoken Languages: </span>
                                <span className="ml-2 text-gray-400">{movie.spoken_languages && movie.spoken_languages.length>0 ? movie.spoken_languages.map((l)=>l.english_name).join(", "): "N/A"}</span>
                            </li>
                    ...
                    <div className="flex-1">
                        ...
                        <p className="italic text-gray-400 mb-6">{movie.tagline||"No tagline avilable"}</p>
                ...
                <div className="p-8">
                    ...
                    <div >
                        ...
                        {recommendations.slice(0,10).map((rec)=>(
                            <SwiperSlide key={rec.id} className="bg-[#232323] rounded-lg overflow-hidden hover:scale-103 transition">
                                <Link to={`/movie/${rec.id}`}>
                                    <img src={`https://image.tmdb.org/t/p/w300${rec.poster_path}` } className="w-full h-70 object-cover"/>
                                    <div className="p-2">
